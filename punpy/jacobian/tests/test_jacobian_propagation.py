...
class TestMCPropagation(unittest.TestCase):
    ...
    def test_propagate_random_1D(self):
        ...
        uf = prop.propagate_random(function,xs,xerrs,corr_between=np.ones((2,2)),Jx=Jx)
    ...
    def test_propagate_random_1D_3var(self):
        ...
        ufc,ucorrc = prop.propagate_random(functionc,xsc,xerrsc,return_corr=True,Jx=Jx)
    ...
    def test_propagate_random_3D_2out(self):
        prop = JacobianPropagation()
        ufd,ucorrd,corr_out = prop.propagate_random(functiond,xsd,xerrsd,
        ...
        ufd,ucorrd,corr_out = prop.propagate_random(functiond,xsd,xerrsd,
                                                    corr_between=corr_d,return_corr=True,
                                                    output_vars=2)
    ...
    def test_propagate_systematic_1D(self):
        ...
        uf,ucorr = prop.propagate_systematic(function,xs,[x1err,None],
                                             ...
                                             fixed_corr_var=True,Jx=Jx)
        npt.assert_allclose(ucorr,corr,atol=0.0001)

        uf,ucorr = prop.propagate_systematic(function,xs,xerrs,return_corr=True,Jx=Jx)
        npt.assert_allclose(ucorr,np.ones_like(ucorr),atol=0.01)
        npt.assert_allclose(uf,yerr_uncorr_1order,atol=0.01)

        uf = prop.propagate_systematic(function,xs,xerrs,corr_between=np.ones((2,2)),Jx=Jx)
        npt.assert_allclose(uf,yerr_corr_1order,atol=0.01)
    ...
    def test_propagate_systematic_1D_3var(self):
        ...
        ufc,ucorrc = prop.propagate_systematic(functionc,xsc,xerrsc,return_corr=True,Jx=Jx)
        npt.assert_allclose(ucorrc,np.ones_like(ucorrc),atol=0.01)
        npt.assert_allclose(ufc,yerr_uncorrc,rtol=0.01)

        ufc = prop.propagate_systematic(functionc,xsc,xerrsc,corr_between=corr_c,Jx=Jx)
        npt.assert_allclose(ufc,yerr_corrc,rtol=0.01)
    ...
    def test_propagate_cov_1D(self):
        ...
        cov = [util.convert_corr_to_cov(np.eye(len(xerr.flatten())),xerr) for xerr in
               xerrs]
        ...
        uf,ucorr = prop.propagate_cov(function,xs,cov,return_corr=True,
                                      corr_between=np.ones((2,2)),Jx=Jx)
    ...
    def test_propagate_cov_2D(self):
        ...
        covb = [util.convert_corr_to_cov(np.eye(len(xerrb.flatten())),xerrb) for
                xerrb in xerrsb]
        ufb,ucorrb = prop.propagate_cov(functionb,xsb,covb,return_corr=True,Jx_diag=True)
        npt.assert_allclose(ucorrb,np.eye(len(ucorrb)),atol=0.01)
        npt.assert_allclose(ufb,yerr_uncorrb,rtol=0.01)

        covb = [util.convert_corr_to_cov(
            np.ones((len(xerrb.flatten()),len(xerrb.flatten())))+np.eye(
                len(xerrb.flatten())),xerrb) for xerrb in xerrsb]
        ufb,ucorrb = prop.propagate_cov(functionb,xsb,covb,return_corr=True,Jx_diag=True)
        ...
        ufb,ucorrb = prop.propagate_cov(functionb,xsb,covb,return_corr=True,
                                        corr_between=np.ones((2,2)),Jx_diag=True)
        npt.assert_allclose(ucorrb,np.eye(len(ucorrb)),atol=0.01)
        npt.assert_allclose(ufb,yerr_corrb,atol=0.03)
    ...
    def test_propagate_cov_1D_3var(self):
        ...
        covc = [util.convert_corr_to_cov(np.eye(len(xerrc.flatten())),xerrc) for
                xerrc in xerrsc]
        ufc,ucorrc = prop.propagate_cov(functionc,xsc,covc,return_corr=True,Jx=Jx)
        npt.assert_allclose(ucorrc,np.eye(len(ucorrc)),atol=0.01)
        npt.assert_allclose(ufc,yerr_uncorrc,rtol=0.01)

        covc = [util.convert_corr_to_cov(
            np.ones((len(xerrc.flatten()),len(xerrc.flatten())))+np.eye(len(xerrc)),
            xerrc) for xerrc in xerrsc]
        ufc,ucorrc = prop.propagate_cov(functionc,xsc,covc,return_corr=True,Jx=Jx)
    ...
    def test_propagate_cov_3D_2out(self):
        ...
        covd = [util.convert_corr_to_cov(np.eye(len(xerrd.flatten())),xerrd) for
                xerrd in xerrsd]
        ufd,ucorrd,corr_out = prop.propagate_cov(functiond,xsd,covd,return_corr=True,
                                                 output_vars=2)
        npt.assert_allclose(ucorrd[0],np.eye(len(ucorrd[0])),atol=0.01)
        npt.assert_allclose(ufd,yerr_uncorrd,rtol=0.01)

        covd = [util.convert_corr_to_cov(
            np.ones((len(xerrd.flatten()),len(xerrd.flatten()))),xerrd) for xerrd in
                xerrsd]
        ufd,ucorrd,corr_out = prop.propagate_cov(functiond,xsd,covd,return_corr=True,
                                                 output_vars=2)
        npt.assert_allclose(ucorrd[1],np.ones_like(ucorrd[1]),atol=0.01)
    ...
    def test_propagate_syst_corr_2D(self):
        ...
        ufb,ucorrb = prop.propagate_systematic(functionb,xsb,xerrsb,corr_x=corrb,
                                               repeat_dims=0,corr_axis=1,
                                               return_corr=True)
        ufb,ucorrb2 = prop.propagate_systematic(functionb,xsb,xerrsb,
                                                corr_x=["rand","rand"],repeat_dims=0,
                                                corr_axis=1,return_corr=True)
        npt.assert_allclose(ucorrb,np.eye(len(ucorrb)),atol=0.01)
        npt.assert_allclose(ucorrb2,np.eye(len(ucorrb2)),atol=0.01)
        npt.assert_allclose(ufb,yerr_uncorrb,rtol=0.01)

        corrb = [np.ones((len(xerrb[0].flatten()),len(xerrb[0].flatten()))) for xerrb
                 in xerrsb]
        ufb,ucorrb = prop.propagate_systematic(functionb,xsb,xerrsb,corr_x=corrb,
                                               repeat_dims=0,corr_axis=1,
                                               return_corr=True)
        ufb,ucorrb2 = prop.propagate_systematic(functionb,xsb,xerrsb,
                                                corr_x=["syst",None],repeat_dims=0,
                                                corr_axis=1,return_corr=True)
        npt.assert_allclose(ucorrb,np.ones_like(ucorrb),atol=0.01)
    ...
    def test_propagate_syst_corr_3D_2out(self):
        ...
        corrd = [np.ones((len(xerrd[:,0,0].flatten()),len(xerrd[:,0,0].flatten())))
                 for xerrd in xerrsd]
        ufd,ucorrd,corr_out = prop.propagate_systematic(functiond,xsd,xerrsd,
                                                        corr_x=corrd,
                                                        return_corr=True,corr_axis=0,
